...
def _safe_entropy(value: str) -> float:
    """Calculate Shannon entropy for a short string without errors on empty input."""
    if not value:
        return 0.0
    counts: dict[str, int] = {}
    for char in value:
...
def extract_features(df: pd.DataFrame) -> pd.DataFrame:
    """Validate raw event columns and produce model-ready deterministic features."""
    ...
    ordered_protocols = [f"proto_{protocol}" for protocol in PROTOCOLS]
    out = pd.concat([out, protocol_dummies[ordered_protocols]], axis=1)
    return out
